      ...
      <Modal className="mobile-fullscreen-modal" show={Boolean(editingRate)} onClose={closeEditModal} size="lg">
        ...
        <Modal.Body>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <TextInput type="number" placeholder="Charge (P)" value={editForm.chargeAmount} onChange={e => setEditForm(f => ({ ...f, chargeAmount: e.target.value }))} sizing="lg" />
            <TextInput type="number" placeholder="# Helpers" value={editForm.helpersCount} onChange={e => setEditForm(f => ({ ...f, helpersCount: e.target.value }))} sizing="lg" />
          </div>
        </Modal.Body>
        <Modal.Footer>
          <Button size="xl" onClick={handleUpdate}>Save</Button>

            ...
            {inventory.map(item => (
              <div key={item.id} className="bg-white rounded-2xl shadow p-5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => setCart(c => ({ ...c, [item.id]: Math.max(0, (c[item.id] || 0) - 1) }))} className="w-10 h-10 rounded-full bg-gray-200 text-xl font-bold">-</button>
                  <span className="text-2xl font-bold w-8 text-center">{cart[item.id] || 0}</span>
                  <button onClick={() => setCart(c => ({ ...c, [item.id]: Math.min(item.availableQuantity, (c[item.id] || 0) + 1) }))} className="w-10 h-10 rounded-full bg-blue-600 text-white text-xl font-bold">+</button>
                </div>
              </div>
            ))}
